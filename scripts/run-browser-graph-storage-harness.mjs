...
async function waitForResult(cdp, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    const evaluated = await cdp.send("Runtime.evaluate", {
      expression: "JSON.stringify(globalThis.__MINDMAP_GRAPH_STORAGE_HARNESS_RESULT__ ?? null)",
      returnByValue: true,
      awaitPromise: true,
    });
  ...
  throw new Error("browser_graph_storage_harness_result_timeout");
}

async function main() {
  execFileSync("npm", ["run", "build:graph-storage-harness"], { stdio: "inherit" });
  const serverPort = await freePort();
  const debugPort = await freePort();
  const url = `http://127.0.0.1:${serverPort}/`;
  const profile = await mkdtemp(join(tmpdir(), "mindmap-graph-storage-chrome-"));
  const server = spawn(process.execPath, ["tools/browser-graph-storage-harness/server.mjs"], {
    stdio: ["ignore", "pipe", "pipe"],
    env: { ...process.env, MINDMAP_GRAPH_STORAGE_HARNESS_PORT: String(serverPort) },
  });
  let chrome;
  try {
    await waitForLine(server, "MINDMAP_GRAPH_STORAGE_HARNESS_READY", 10_000);
    ...
    try {
      ...
      if (!result?.ok) throw new Error(`browser_graph_storage_harness_failed:${JSON.stringify(result)}`);
      for (const field of [
        "browserIndexedDb",
        "atomicCommit",
        "reopen",
        ...
        "workspaceIsolation",
        "abortRollback",
        "runAdapterCompatibility",
        "runOnlyRefusal",
      ]) {
        if (result[field] !== true) throw new Error(`browser_graph_storage_harness_missing_proof:${field}`);
      }
      if (!/^[a-f0-9]{64}$/.test(result.snapshotHash ?? "")) {
        throw new Error("browser_graph_storage_harness_invalid_snapshot_hash");
      }
    ...
    await stopChild(server, "graph-storage-harness-server");
