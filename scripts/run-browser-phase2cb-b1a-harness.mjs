...
async function waitForResult(cdp, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    const evaluated = await cdp.send("Runtime.evaluate", {
      expression: "JSON.stringify(globalThis.__MINDMAP_PHASE2CB_B1A_HARNESS_RESULT__ ?? null)",
      returnByValue: true,
      awaitPromise: true,
    });
  ...
  throw new Error("browser_phase2cb_b1a_harness_result_timeout");
}

async function main() {
  execFileSync("npm", ["run", "build:phase2cb-b1a-harness"], { stdio: "inherit" });
  const serverPort = await freePort();
  const debugPort = await freePort();
  const url = `http://127.0.0.1:${serverPort}/`;
  const profile = await mkdtemp(join(tmpdir(), "mindmap-phase2cb-b1a-chrome-"));
  const server = spawn(process.execPath, ["tools/browser-phase2cb-b1a-harness/server.mjs"], {
    stdio: ["ignore", "pipe", "pipe"],
    env: { ...process.env, MINDMAP_PHASE2CB_B1A_HARNESS_PORT: String(serverPort) },
  });
  let chrome;
  try {
    await waitForLine(server, "MINDMAP_PHASE2CB_B1A_HARNESS_READY", 10_000);
    ...
    try {
      ...
      if (!result?.ok) throw new Error(`browser_phase2cb_b1a_harness_failed:${JSON.stringify(result)}`);
      for (const field of [
        "browserIndexedDb",
        ...
        if (result[field] !== true) throw new Error(`browser_phase2cb_b1a_harness_missing_proof:${field}`);
      }
      if (result.exactSourceOpened !== false || result.actualMigrationPerformed !== false) {
        throw new Error("browser_phase2cb_b1a_harness_boundary_violation");
      }
      if (!/^[a-f0-9]{64}$/.test(result.portablePlanHash ?? "")
        || !/^[a-f0-9]{64}$/.test(result.targetSnapshotHash ?? "")) {
        throw new Error("browser_phase2cb_b1a_harness_invalid_hash");
      }
    ...
    await stopChild(server, "phase2cb-b1a-harness-server");
