...
async function waitForResult(cdp, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    const evaluated = await cdp.send("Runtime.evaluate", {
      expression: "JSON.stringify(globalThis.__MINDMAP_PHASE2CC_C3_HARNESS_RESULT__ ?? null)",
      returnByValue: true,
      awaitPromise: true,
    });
  ...
  throw new Error("browser_phase2cc_c3_harness_result_timeout");
}

async function main() {
  execFileSync("npm", ["run", "build:phase2cc-c3-harness"], { stdio: "inherit" });
  const serverPort = await freePort();
  const debugPort = await freePort();
  const url = `http://127.0.0.1:${serverPort}/`;
  const profile = await mkdtemp(join(tmpdir(), "mindmap-phase2cc-c3-chrome-"));
  const server = spawn(process.execPath, ["tools/browser-phase2cc-c3-harness/server.mjs"], {
    stdio: ["ignore", "pipe", "pipe"],
    env: { ...process.env, MINDMAP_PHASE2CC_C3_HARNESS_PORT: String(serverPort) },
  });
  let chrome;
  try {
    await waitForLine(server, "MINDMAP_PHASE2CC_C3_HARNESS_READY", 10_000);
    ...
    try {
      ...
      if (!result?.ok) throw new Error(`browser_phase2cc_c3_harness_failed:${JSON.stringify(result)}`);
      for (const field of [
        "packagedRuntimeModule",
        "browserIndexedDb",
        "activeGenerationResolved",
        "deterministicReload",
        "immutableSealVerified",
        "snapshotHashVerified",
        "stalePointerRejected",
        "missingRegistryFailClosed",
        "noFallback",
        "readOnlyNoMutation",
        "reqObsTrace",
        ...
        if (result[field] !== true) throw new Error(`browser_phase2cc_c3_harness_missing_proof:${field}`);
      }
      for (const field of [
        "exactSourceOpened",
        "backupAccessed",
        ...
        if (result[field] !== false) throw new Error(`browser_phase2cc_c3_harness_unsafe_proof:${field}`);
      }
      if (!/^[a-f0-9]{64}$/.test(result.targetSnapshotHash ?? "")) {
        throw new Error("browser_phase2cc_c3_harness_invalid_snapshot_hash");
      }
    ...
    await stopChild(server, "phase2cc-c3-harness-server");
