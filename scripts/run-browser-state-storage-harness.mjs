...
function findChrome() {
  ...
  if (process.platform === "darwin") {
    ...
    const check = spawnSync("test", ["-x", mac]);
    if (check.status === 0) return mac;
  }
...
async function waitForHarnessResult(cdp, timeoutMs) {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    const evaluated = await cdp.send("Runtime.evaluate", {
      expression: "JSON.stringify(globalThis.__MINDMAP_STORAGE_HARNESS_RESULT__ ?? null)",
      returnByValue: true,
      awaitPromise: true,
    });
  ...
  throw new Error("browser_storage_harness_result_timeout");
}

async function main() {
  execFileSync("npm", ["run", "build:storage-harness"], { stdio: "inherit" });
  const serverPort = await freePort();
  const debugPort = await freePort();
  const url = `http://127.0.0.1:${serverPort}/`;
  const profile = await mkdtemp(join(tmpdir(), "mindmap-storage-harness-chrome-"));
  const chromePath = findChrome();
  const server = spawn(process.execPath, ["tools/browser-state-storage-harness/server.mjs"], {
    stdio: ["ignore", "pipe", "pipe"],
    env: { ...process.env, MINDMAP_STORAGE_HARNESS_PORT: String(serverPort) },
  });
  let chrome;
  try {
    await waitForLine(server, "MINDMAP_STORAGE_HARNESS_READY", 10_000);
    ...
    try {
      ...
      const result = await waitForHarnessResult(cdp, HARNESS_TIMEOUT_MS);
      if (!result?.ok) throw new Error(`browser_storage_harness_failed:${JSON.stringify(result)}`);
      for (const field of ["browserIndexedDb", "atomicCommit", "reopen", "idempotency", "workspaceIsolation", "abortRollback", "upgradeRollback"]) {
        if (result[field] !== true) throw new Error(`browser_storage_harness_missing_proof:${field}`);
      }
      if (!/^[a-f0-9]{64}$/.test(result.snapshotHash ?? "")) {
        throw new Error("browser_storage_harness_invalid_snapshot_hash");
      }
    ...
    await stopChild(server, "storage-harness-server");
