...
import type { Phase2CbMappingCandidate } from "../../migration/phase2cb-contracts.ts";

interface Bootstrap {
  readonly runId: string;
  readonly sourceKind: "sanitized-rehearsal" | "exact-source";
  readonly exactSourceOpened: boolean;
  readonly sourceSnapshot: Readonly<{ exists: boolean; sizeBytes: number; sha256: string; modifiedTimeMs?: number }>;
  readonly candidate: Phase2CbMappingCandidate;
}
...
declare global {
  interface Window {
    __MINDMAP_PHASE2CB_B1B_HARNESS_RESULT__?: HarnessResult;
    __MINDMAP_PHASE2CB_B1B_HARNESS_EVIDENCE__?: unknown;
  }
}
...
function readBootstrap(): Bootstrap {
  const element = document.querySelector<HTMLScriptElement>("#mindmap-b1b-bootstrap");
  if (element?.textContent) return JSON.parse(element.textContent) as Bootstrap;
  return {
    runId: "sanitized-browser-rehearsal",
    sourceKind: "sanitized-rehearsal",
...
if (downloadButton) {
  downloadButton.addEventListener("click", () => {
    ...
    anchor.href = url;
    anchor.download = "mindmap-phase2cb-b1b-sanitized-diagnostic.json";
    anchor.click();
...
function show(result: HarnessResult, evidence?: unknown): void {
  window.__MINDMAP_PHASE2CB_B1B_HARNESS_RESULT__ = result;
  window.__MINDMAP_PHASE2CB_B1B_HARNESS_EVIDENCE__ = evidence;
  ...
  document.title = result.ok
    ? "PASS — MindMap Phase 2C-B1b Browser Harness"
    : "FAIL — MindMap Phase 2C-B1b Browser Harness";
}
...
function installNetworkGuards(onCall: (path: string) => never): boolean {
  ...
  try {
    Object.defineProperty(navigator, "sendBeacon", {
      value: guard("navigator.sendBeacon"), configurable: true, writable: false,
    });
  } catch { installed = false; }
  return installed;
}
...
async function main(): Promise<void> {
  try {
    ...
    const source = new PreparedPhase2CbB1bSourceAdapter({
      sourceKind: bootstrap.sourceKind,
      sourceId: bootstrap.sourceKind === "exact-source" ? "exact-source-prepared" : "sanitized-browser-rehearsal",
      sourceSnapshot: bootstrap.sourceSnapshot,
      candidate: bootstrap.candidate,
      exactSourceOpened: bootstrap.exactSourceOpened,
    });
    ...
    const result = await runPhase2CbB1bHarness({
    ...
    if ("stop" in result) throw new Error(`b1b_browser_stop:${result.stop.code}:${result.stop.message}`);
    const requiredSteps = new Set([
      "freeze_manifest", "offline_preflight", "source_snapshot_before", "read_only_extraction",
      "deterministic_planning", "target_freshness", "target_creation", "transactional_write",
      "verification", "source_snapshot_after", "cleanup", "complete",
    ]);
    ...
    if (!reqObsTrace) throw new Error("b1b_browser_req_obs_trace_incomplete");
    ...
    show({
      ...
      automaticRetryAllowed: result.evidence.automaticRetryAllowed,
      reqObsTrace,
